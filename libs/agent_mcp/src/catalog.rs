...
pub fn tools() -> Vec<Value> {
    ...
    tools.extend([
        tool("list_windows", "List visible/minimized top-level remote Windows windows without walking UIA or running OCR. Filter title/process_name case-insensitively (substring unless exact=true); ambiguous matches require choosing a window_id. IDs are session-scoped and expire after 30 seconds. Does not launch apps or enumerate unlaunched pinned icons.", true, true, vec![("title",string()),("process_name",string()),("exact",boolean())], &[]),
        tool("get_foreground_window", "Read the remote Windows foreground title, process identity and desktop bounds without a UIA tree or OCR. Observation only; does not prove input/control focus or completion.", true, true, vec![], &[]),
        ...
        tool("get_ui_tree", "Read bounded remote Windows UIA controls in scope foreground_window (default) or taskbar (including exposed tray controls). Icon labels depend on accessibility providers, not OCR. Coordinates are original display pixels; IDs expire with the snapshot.", true, true, vec![("display",integer(0,63)),("scope",choice(&["foreground_window","taskbar"]))], &[]),
        tool("get_ui_state", "Return a screenshot and optionally UIA/OCR observations. include_uia/include_ocr default true; disable unavailable or unnecessary sources to avoid repeated scanning. OCR cannot identify unlabeled icons.", true, true, {let mut p=observation();p.extend([("scope",choice(&["foreground_window","taskbar"])),("include_uia",boolean()),("include_ocr",boolean())]);p}, &[]),
        ...
        tool("find_text", "Find text using PP-OCRv4 on a fresh remote frame; use match_index to disambiguate before click_text.", true, true, {
            let mut p = query(); p.retain(|(k,_)| !["automation_id","control_type","scope"].contains(k)); p
        }, &["text"]),

...
pub fn tools() -> Vec<Value> {
    ...
    let mut tools = vec![
        ...
        tool("connect_device","Connect through RustDesk and wait up to timeout_ms for authentication, including reused sessions. Cached credentials are tried automatically. Returns a stable session UUID, connected, needs_password and authentication state. A timeout with authenticating is still pending, not a password requirement. password_or_approval means a local prompt; remote approval may still complete it. headless is opt-in. Authentication is never bypassed.",false,false,
            vec![("device_id",json!({"type":"string","minLength":1,"maxLength":256})),("kind",choice(&["desktop","terminal","files"])),("headless",boolean()),("force_relay",boolean()),("timeout_ms",integer(100,120000))],&["device_id"]),
        tool("get_connection_info","Read authentication state, displays and permissions. Poll the same session while connecting/authenticating or waiting_remote_approval; do not open a different session just because authentication is pending. two_factor_required and os_login_required are distinct from password_required. Use get_recent_events for details.",true,true,vec![],&[]),
        tool("disconnect_device","Close this session and release its MCP caches.",true,false,vec![],&[]),
        tool("input_password","Submit credentials for an explicit password or OS login prompt, then wait for authentication on the same session. A handshake alone is not a password prompt. password_or_approval also allows waiting for remote approval. Does not persist the password.",true,false,vec![("password",string()),("os_username",string()),("os_password",string())],&["password"]),
        ...
        tool("terminal_open","Open or reattach a PTY using the same terminal_id. Wait up to timeout_ms (default 3000) for the remote opened acknowledgement; queued/starting is not ready. Repeating an active ID only observes it. After reconnect explicitly reopen the same ID; a lost service may create a new shell, never replay commands.",true,false,
            vec![("terminal_id",integer(1,2147483647)),("rows",integer(1,500)),("cols",integer(1,1000)),("timeout_ms",integer(0,10000))],&["terminal_id"]),
        ...
        tool("terminal_output","Read bounded PTY output using byte cursors. Returns readiness state, last remote details, generation, raw base64, best-effort UTF-8 text, next_cursor and truncated. Reset cursor to zero when reopening changes generation.",true,true,
        ...
        tool("file_list","List a REMOTE directory on a files session, never the controller or agent workspace. Wait up to timeout_ms (default 3000) and return parsed entries plus next_offset for file_directory. If still pending, use after_cursor and file_dir events. Large events carry a bounded page and paginated:true.",true,true,
            vec![("path",string()),("include_hidden",boolean()),("timeout_ms",integer(0,10000))],&["path"]),
        ...
        tool("file_transfer","Upload or download through RustDesk file transfer. Upload source and download destination are absolute paths in the RustDesk MCP CONTROLLER process filesystem, not necessarily the agent/stdio/container filesystem. The other path is remote. Upload checks controller access before queueing. Returns a job ID and path metadata; completion is asynchronous. Existing files require file_confirm_override. For bytes from another agent filesystem, use write_workspace_file on a terminal session.",true,false,
